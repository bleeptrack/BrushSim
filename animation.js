...
function newMask(event){
    let vec = event.delta.multiply(-1);
    let d = vec.rotate(90);
    maskm = new Path();
    maskl = new Path();
    if(debug){
...
function move(event){
    moveBrushhead(event.point.add(event.delta.rotate(90)));
    
}

function moveBrushhead(nextPoint){
    maskm.firstSegment.point = nextPoint;
    for (let i = 0; i < points - 1; i++) {
        let segment = maskm.segments[i];
    ...
    maskl.firstSegment.point = nextPoint;
    for (let i = 0; i < points - 1; i++) {
        let segment = maskl.segments[i];
        let nextSegment = segment.next;
        let vector = segment.point.subtract(nextSegment.point);
        vector.length = length;
        nextSegment.point = segment.point.subtract(vector);
...
//move brush and draw
function drag(event) {
    let step = event.delta.normalize(getRandomInt(widthMin, widthMax));
    step.angle += 90;
    ...
    moveBrushhead(event.point.add(step));
    ...
    for(let i = pathl.segments.length-1; i>pathl.segments.length-60 && i>0; i--){
        let p = pathl.segments[i].point;
        let m = pathm.segments[i].point;
        let near = maskm.getNearestPoint(p);
        
        let inner = p.subtract(m);
        let outer = near.subtract(m);
        let rest = outer.subtract(inner);
    ...
    for(let i = pathr.segments.length-1; i>pathr.segments.length-60 && i>0; i--){
        let p = pathr.segments[i].point;
        let m = pathm.segments[i].point;
        let near = maskl.getNearestPoint(p);
